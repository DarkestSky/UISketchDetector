...
def do_train(cfg, model, resume=False):
    ...
    data_loader = build_detection_train_loader(cfg)     # AspectRatioGroupedDataset
    ...
    with EventStorage(start_iter) as storage:
        for data, iteration in zip(data_loader, range(start_iter, max_iter)):
            ...
            # 'image' 是 Tensor, torch.Size([3, 640, 360])
            # 'instances' 是 <class 'detectron2.structures.instances.Instances'>
            # 包含 'gt_boxes' 和 'gt_classes' 分别表示 bbox 和对应的 class
...
def setup(args):
    ...
    # ! 此处定义 synz 的 annotations json 文件, 后续新建 COCO 实例可以读取到数据
