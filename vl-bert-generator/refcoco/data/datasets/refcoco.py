...
class RefCOCO(Dataset):
    def __init__(self, image_set, root_path, data_path, boxes='gt', proposal_source='official',
        ...
        self.proposal_source = proposal_source      # official
        self.boxes = boxes      # 'gt'
        self.test_mode = test_mode
        self.category_to_idx = {c: i for i, c in enumerate(categories)}
        self.data_path = data_path      # './data/coco'
        self.root_path = root_path      # ./
        self.transform = transform
        self.image_sets = [iset.strip() for iset in image_set.split('+')]       # 'train'
        self.coco = COCO(annotation_file=os.path.join(data_path, coco_annot_files['train2014']))        # ./data/coco/annotations/instances_train2014.json
        self.refer = REFER(data_path, dataset='refcoco+', splitBy='unc')
        self.refer_ids = []
        for iset in self.image_sets:
            self.refer_ids.extend(self.refer.getRefIds(split=iset))     # refer 中 data['refs'] 中所有分到当前任务的 id
        self.refs = self.refer.loadRefs(ref_ids=self.refer_ids)         # refs id 对应的数据
        ...
        self.zip_mode = zip_mode        # false
        self.cache_mode = cache_mode    # false
        self.cache_db = cache_db        # true
        self.ignore_db_cache = ignore_db_cache      # true
        self.aspect_grouping = aspect_grouping      # true
        self.cache_dir = os.path.join(root_path, 'cache')
        self.add_image_as_a_box = add_image_as_a_box
        self.mask_size = mask_size          # default (14, 14)
    ...
    def __getitem__(self, index):
        ...
        if self.transform is not None:
            if not self.test_mode:
                boxes = torch.cat((gt_box[None], boxes), 0)     # gt_box 取自 refcoco+, 是本条数据对应的一个 box; boxes 取自 COCO, 是 本条数据所处图片中的所有 box
            image, boxes, _, im_info, flipped = self.transform(image, boxes, None, im_info, flipped)    # image, boxes, masks, im_info, flipped
            if not self.test_mode:
                gt_box = boxes[0]
                boxes = boxes[1:]
